...
class PHPJasperXML {
    ...
    public function load_xml_file($file){
      ...
      $this->pageBandsInfo = array('pageHeight'=>$arrayXml['@attributes']['pageHeight'],
                                   'topMargin'=>$arrayXml['@attributes']['topMargin'],
                                   'bottomMargin'=>$arrayXml['@attributes']['bottomMargin'],
                                   'summaryHeight'=>$arrayXml['summary']['band']['@attributes']['height'],
                                   'pageHeaderHeight'=>$arrayXml['pageHeader']['band']['@attributes']['height'],
                                   'detailHeight'=>$arrayXml['detail']['band']['@attributes']['height'],
                                   'groupHeight'=>$arrayXml['group']['groupHeader']['band']['@attributes']['height'],
                                   'groupName'=>$arrayXml['group']['@attributes']['name']);
       unset($arrayXml);
...
//        $this->pointer[]=array("type"=>"MultiCell","width"=>$data->reportElement["width"],"height"=>$height,"txt"=>$data->text,"border"=>$border,"align"=>$align,"fill"=>$fill,"hidden_type"=>"statictext","soverflow"=>$stretchoverflow,"poverflow"=>$printoverflow,"rotation"=>$rotation);
    ...
    public function element_image($data) {
        ...
        switch($data[scaleImage]) {
            case "FillFrame":
              $path = getcwd()."/module/Application/view/application/report/phpJasperXml/"; //@igormoura
     ...
     $this->offsetposition=-1;
        ...
        foreach ($this->arraysummary as $out) {
           ...
           $this->report_count++;
             ...
             //echo '<pre>', print_r($this->pageBandsInfo), '</pre>';
             
             $totalBandsHeight = ($this->pageBandsInfo['pageHeaderHeight']
                                + $this->pageBandsInfo['totalDetailHeight']
                                + $this->pageBandsInfo['totalGroupHheight']
                                + $this->pageBandsInfo['topMargin']
                                + $this->pageBandsInfo['bottomMargin']);
             //echo $totalBandsHeight;
             
             $currentPage=$this->pdf->GetPage();
             
             // se for mais de 1 página
             if($currentPage > 0){//divide a soma de altura das bandas pelo número de páginas
                 $totalBandsHeight = ($totalBandsHeight / $currentPage);
             }
             
             $espacoLivre = ($this->pageBandsInfo['pageHeight'] - $totalBandsHeight);
...
//                $arrdata[$num]=str_replace('"',"",$out);
    ...
    public function includeSubReport($d,$arrdata,$current_y){ 
               ...
               $srxml=  simplexml_load_file($path.$d['subreportExpression']);
