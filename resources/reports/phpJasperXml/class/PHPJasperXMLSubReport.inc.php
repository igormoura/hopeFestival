...
class PHPJasperXMLSubReport{
    ...
    public function connect($db_host,$db_user,$db_pass,$db_or_dsn_name,$cndriver="mysql",$debug = false) {
    ...
    $this->cndriver=$cndriver;
        if($cndriver=="mysql") {
            ...
            if(!$this->con) {
                ...
                $this->myconn = new PDO("mysql:host={$db_host};dbname={$db_or_dsn_name};charset=utf8", "{$db_user}", "{$db_pass}", array(PDO::ATTR_EMULATE_PREPARES => false, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
        ...
        if(!$this->connect($host,$user,$password,$db_or_dsn_name,$cndriver,true))	//connect database
        {
        ...
        if($cndriver=="odbc") {
            ...
            $stmt = $this->myconn->prepare($this->sql);
            
            $stmt->execute();

            $dadosSelect = $stmt->fetchAll();
            
            //while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            foreach ($dadosSelect as $key => $row) {
...
//echo $sql."<br/><br/>";
    ...
    //while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    foreach ($dadosSelect as $keyData => $row) {
